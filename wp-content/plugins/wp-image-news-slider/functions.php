 ...
 * Ladda script
 */
function dazake_news_load_script() {
	if (!is_admin()) {
		wp_register_script('jquery', plugins_url().'/wp-image-news-slider/mobile/jquery-1.7.1.min.js');
		wp_register_script('easing', plugins_url().'/wp-image-news-slider/mobile/easing.js');
		wp_register_script('trans-banner', plugins_url().'/wp-image-news-slider/mobile/trans-banner.min.js');
		wp_register_script('main', plugins_url().'/wp-image-news-slider/mobile/main.js');
		...
		wp_enqueue_script('trans-banner');
		wp_enqueue_script('main');

		/**
		 * load stylesheet
		 */
		wp_enqueue_style( 'style', plugins_url().'/wp-image-news-slider/mobile/mobile.css' );
	}
}

if(strstr($_SERVER['HTTP_USER_AGENT'],"iPad") || strstr($_SERVER['HTTP_USER_AGENT'],"iPhone")) {
	add_action('init', 'dazake_news_load_script');
}
...
$xml_settings = '<config fontface="'.$ops['ss_font'].'" bannerWidth="'.$ops['bannerWidth'].'" bannerHeight="'.$ops['bannerHeight'].'" slideshowbackgroundcolor="'.'0x'.$ops['slideshow_bgcolor'].'" descriptionbackgroundcolor="'.'0x'.$ops['desc_bgcolor'].'" descriptionbackgroundwidth="'.$ops['desc_bgwidth'].'" timerColor_1="'.'0x'.$ops['timerColor_1'].'" timerColor_2="'.'0x'.$ops['timerColor_2'].'" buttonsbackgroundcolor="'.'0x'.$ops['button_bgcolor'].'" buttonsXPos="'.$ops['control_xpos'].'" buttonsYPos="'.$ops['control_ypos'].'" autoplay="'.$ops['auto_play'].'" showbuttons="'.$ops['show_buttons'].'" autohidebuttons="'.$ops['autohide_buttons'].'" showbuttonshandcursor="'.$ops['show_buttonshandcursor'].'" transitiontimedelay="'.$ops['transition_timedelay'].'" buttonrolloverbackgroundcolor="'.'0x'.$ops['button_rollovrbgcolor'].'" buttonrolloversymbolcolor="'.'0x'.$ops['button_rollovrsymbalcolor'].'" buttonrolloutbackgroundcolor="'.'0x'.$ops['button_rolloutbgcolor'].'" buttonrolloutsymbolcolor="'.'0x'.$ops['button_rolloutsymbalcolor'].'" pricetagImage="'.IMN_PLUGIN_URL.'/'.$ops['priceButton_Path'].'" pricebuttonXPos="'.$ops['pricebuttonXPos'].'" pricebuttonYPos="'.$ops['pricebuttonYPos'].'" pricebuttonshow="'.$ops['show_price'].'" ImageScaling="'.$ops['image_scalling'].'" randomtransition="'.$ops['random_transition'].'" transitionnumber="'.$ops['transition_type'].'" target="'.$ops['target'].'" showreadmore="'.$ops['show_readmore'].'" readmore_text="'.$ops['readmore_text'].'" readmore_size="'.$ops['readmore_size'].'" readmore_font="'.$ops['readmore_font'].'" readmore_color="'.'0x'.$ops['readmore_color'].'" readmore_underline="'.$ops['readmore_underline'].'" show_caption="'.$ops['show_caption'].'" title_size="'.$ops['title_size'].'" title_color="'.$ops['title_color'].'" title_font="'.$ops['title_font'].'" show_desc="'.$ops['show_desc'].'" desc_size="'.$ops['desc_size'].'" desc_color="'.$ops['desc_color'].'" show_price="'.$ops['show_price'].'"   price_size="'.$ops['price_size'].'" currency_symbol="'.$ops['currency_symbol'].'" wmode="'.$ops['wmode'].'" />';
	return $xml_settings;
}
